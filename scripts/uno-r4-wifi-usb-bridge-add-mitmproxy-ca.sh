...
mv x509_crt_bundle uno-r4-wifi-usb-bridge-trust-with-mitmproxy.edsp-idf-trust
if [[ $(stat --printf="%s" uno-r4-wifi-usb-bridge-trust-with-mitmproxy.edsp-idf-trust) -gt 282624 ]]; then
  echo 'uno-r4-wifi-usb-bridge-trust-with-mitmproxy.edsp-idf-trust is larger than certs map size (0x045000)!' 1>&2
  exit 1;
fi
dd if=uno-r4-wifi-usb-bridge-trust-with-mitmproxy.edsp-idf-trust of="$firmware" bs=4096 seek=11 conv=notrunc
