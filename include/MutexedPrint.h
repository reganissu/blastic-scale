...
public:
  ...
  template<typename... Args> size_t write(Args&&... args) {
    return obj.write(std::forward<Args>(args)...);
  }
  template<typename... Args> size_t print(Args&&... args) {
    return obj.print(std::forward<Args>(args)...);
  }
  template<typename... Args> size_t println(Args&&... args) {
    return obj.println(std::forward<Args>(args)...);
  }
};
